...
namespace Ora.Common.Caching
{
    public class MemoryCacheManager : ILocker, IStaticCacheManager
    {
        ...
        public T Get<T>(string key, Func<T> acquire, int? cacheTime = null)
        {
            ...
            return _provider.Get(key, acquire, TimeSpan.FromMinutes(cacheTime ?? OraCachingDefaults.CacheTime))
                .Value;
        }
        ...
        public async Task<T> GetAsync<T>(string key, Func<Task<T>> acquire, int? cacheTime = null)
        {
            ...
            var t = await _provider.GetAsync(key, acquire, TimeSpan.FromMinutes(cacheTime ?? OraCachingDefaults.CacheTime));
            return t.Value;
        }
